...
pelvisTXValue = 0;
pelvisTXSpeed  = 0;
rHipValue = deg2rad(25.0);
rHipSpeed = deg2rad(0);
lHipValue = deg2rad(-20);
lHipSpeed = deg2rad(0);
rKneeValue = deg2rad(-20.0);
rKneeSpeed = deg2rad(0);
lKneeValue = deg2rad(-30.0);
...
%   model_path = '../Model/WalkerModelTerrain_KneeMagnet.osim';
 model_path = '../Model/WalkerModelTerrain_CustomFeet.osim';
...
osimModel.updCoordinateSet().get('LKnee_rz').setValue(state, lKneeValue);
osimModel.updCoordinateSet().get('LKnee_rz').setLocked(state, lKneeSpeed);


% compute the center of mass and plot
